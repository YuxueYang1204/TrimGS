...
def cull_scan(scan, ply_path, result_ply_file, instance_dir):
    ...
    sampled_masks = []
    for i in tqdm(range(n_images),  desc="Culling point clouds given masks"):
        pose = pose_all[i]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Arguments to evaluate the point clouds.'
    )

    parser.add_argument('--input_pcd', type=str,  help='path to the point clouds to be evaluated')
    parser.add_argument('--scan_id', type=str,  help='scan id of the input point clouds')
    ...
    scan = args.scan_id
    ply_file = args.input_pcd
    print("cull pcd ....")
    result_ply_file = os.path.join(out_dir, "culled_pcd.ply")
    cull_scan(scan, ply_file, result_ply_file, instance_dir=os.path.join(args.mask_dir, f'scan{args.scan_id}'))

    script_dir = os.path.dirname(os.path.abspath(__file__))
    cmd = f"python {script_dir}/eval.py --data {result_ply_file} --scan {scan} --dataset_dir {Offical_DTU_Dataset} --vis_out_dir {out_dir}"
    os.system(cmd)